...
impl Rule for PreferClassDirective {
    ...
    fn run<'a>(&self, ctx: &mut LintContext<'a>) {
        ...
        walk_template_nodes(&ctx.ast.html, &mut |node| {
            ...
            // Components have their own class API; only Svelte special that
            // renders a real DOM node is `<svelte:element>`.
            match el.kind() {
                ...
                match value {
                    AttributeValue::Expression(expr) => {
                        if is_simple_class_ternary(expr)
                            || (!prefer_empty && is_dual_class_ternary(expr))
                        {
                    ...
                    AttributeValue::Concat(parts) => {
                        let boundary_ok =
                            |parts: &[AttributeValuePart], idx: usize, before: bool| -> bool {
                                let adj = if before {
                                    idx.checked_sub(1)
                                } else {
                                    (idx < parts.len() - 1).then(|| idx + 1)
                                };
                        ...
                        for (i, part) in parts.iter().enumerate() {
                            let AttributeValuePart::Expression(expr) = part else {
                                continue;
                            };
                            if is_simple_class_ternary(expr)
                                && boundary_ok(parts, i, true)
                                && boundary_ok(parts, i, false)
                            {
                                ctx.diagnostic(msg, *span);
                            } else if !prefer_empty && is_dual_class_ternary(expr) {
                                let ok = parts.iter().enumerate().all(|(j, p)| {
                                    j == i
                                        || match p {
...
fn is_dual_class_ternary(expr: &str) -> bool {
    split_ternary(expr.trim()).map_or(false, |(_, t, f)| {
        is_single_class_name(t) && is_single_class_name(f)
    })
}
...
fn is_simple_class_ternary(expr: &str) -> bool {
    let Some((_, tb, fb)) = split_ternary(expr.trim()) else {
        return false;
    };
