    ...
    fn check(nodes: &[TemplateNode], cn: &str, iter: bool) -> bool {
        nodes.iter().any(|node| match node {
            TemplateNode::Element(el) => {
                let has = el.attributes.iter().any(|a| {
                    matches!(a,
            ...
            TemplateNode::EachBlock(each) => {
                check(&each.body.nodes, cn, true)
                    || each
                        .fallback
                        .as_ref()
                        .is_some_and(|f| check(&f.nodes, cn, true))
            }
            TemplateNode::IfBlock(ib) => {
                check(&ib.consequent.nodes, cn, iter)
                    || ib
                        .alternate
                        .as_ref()
                        .is_some_and(|alt| check(&[*alt.clone()], cn, iter))
            }
            TemplateNode::AwaitBlock(ab) => [&ab.pending, &ab.then, &ab.catch]
                .iter()
...
impl Rule for ConsistentSelectorStyle {
    ...
    fn run<'a>(&self, ctx: &mut LintContext<'a>) {
        ...
        let check_class_selector = |class_name: &str| -> Option<String> {
            ...
            let (count, ref element_types) = class_usage
                .get(class_name)
                .map(|(c, t)| (*c, t.clone()))
        ...
        for_each_rule_prelude(
            &style.content,
            css_start_in_source,
            |prelude_text, start_in_source, inside_global| {
                // When inside `:global { … }`, only emit if the user opted in.
                if inside_global && !check_global {
                    return;
                }
                ...
                walk_components(&list, check_global, &mut |comp, in_global_body| {
                    ...
                    if in_global_body && !check_global {
                        return;
                    }
                    match comp {
                        Component::Class(atom) => {
                            ...
                            if let Some(suggested) = check_class_selector(name) {
                                ...
                                diagnostics.push((
                                    format!(
                                        "Selector should select by {} instead of class",
                                        suggested
                                    ),
                        ...
                        Component::ID(atom) => {
                            ...
                            if let Some(suggested) = check_id_selector(name) {
                                ...
                                diagnostics.push((
                                    format!(
                                        "Selector should select by {} instead of ID",
                                        suggested
                                    ),
                                    Span::new(sp as u32, end as u32),
                                ));
