...
fn check_expr_for_raw_store(
    ...
    for var in store_vars {
        if expr == var
            ...
            || expr.starts_with(&format!("{}(", var))
        {
            if !expr.contains(&format!("${}", var)) && !expr.contains(&format!("get({})", var)) {
...
impl Rule for RequireStoreReactiveAccess {
    ...
    fn run<'a>(&self, ctx: &mut LintContext<'a>) {
        ...
        if store_vars_map.is_empty() {
            return;
        }
        let store_vars: HashSet<String> = store_vars_map.keys().cloned().collect();
        let const_store_vars: HashSet<String> = store_vars_map
            .iter()
        ...
        let content_offset = tag_text
            .find('>')
            .map(|p| script.span.start as usize + p + 1)
        ...
        if let Some(sem) = ctx.instance_semantic {
            for node in sem.nodes().iter() {
                let AstKind::TemplateLiteral(tl) = node.kind() else {
                    continue;
                };
                for (i, expr) in tl.expressions.iter().enumerate() {
                    ...
                    if tl
                        .quasis
                        .get(i)
                        .is_some_and(|q| q.value.raw.as_str().ends_with('$'))
                    {
                        continue;
                    }
        ...
        for var in &store_vars {
            for (pos, _) in content.match_indices(var.as_str()) {
                ...
                if full_line.contains(&format!("${}", var)) {
                    continue;
                }

                let after_text = if after < content.len() {
                ...
                if after_text.starts_with(':') && !after_text.starts_with("::") {
                    ...
                    if before_check.ends_with('{')
                        || before_check.ends_with(',')
                        || before_check.ends_with('\n')
                    {
                        continue;
                    }
                ...
                if before.ends_with(':') && !after_text.starts_with(']') {
                    continue;
                }

                let consistent_context = ["typeof", "typeof ", "!", "await", "await "]
                    .iter()
                    .any(|p| before.ends_with(p))
                    || ["==", "!=", "&&", "||", "??"]
                        .iter()
                        .any(|p| after_text.starts_with(p))
                ...
                let in_for_in_of = before_trimmed.ends_with(" in")
                    || before_trimmed.ends_with(" of")
                    || before_trimmed.ends_with("\tin")
                    || before_trimmed.ends_with("\tof");
                ...
                if before.ends_with('(') {
                    ...
                    if kw_before.ends_with("if")
                        || kw_before.ends_with("switch")
                        || kw_before.ends_with("while")
                    {
                ...
                for attr in &el.attributes {
                    ...
                    } = attr
                    {
                        ...
                        for var in check_vars {
                            let flag = if dir_name == var.as_str() && is_shorthand {
                                true
                            } else if let Some(eq) = region.find('=') {
                                let val = &region[eq + 1..];
                                val.find('{')
                                    .and_then(|o| val.find('}').map(|c| val[o + 1..c].trim()))
                                    .is_some_and(|expr| {
                                        expr == var.as_str() && !expr.starts_with('$')
                                    })
                            } else {
                                false
                            };
                    ...
                    if let Attribute::Spread { span } = attr {
                        ...
                        for var in &store_vars_clone {
                            if has_word_boundary_match(region, var)
                                && !region.contains(&format!("${}", var))
                            {
                                ctx.diagnostic(RAW_STORE_MSG, *span);
                            }
                        }
...
fn expression_references_any(expr: &Expression<'_>, candidates: &HashSet<String>) -> bool {
    match expr {
        ...
        Expression::ParenthesizedExpression(p) => {
            expression_references_any(&p.expression, candidates)
        }
